...
const handleImgError = (e: Event) => {
  ...
  if (target instanceof HTMLImageElement && !target.dataset.fallbackApplied) {
    ...
    fallbackDiv.className =
      "flex flex-col items-center justify-center p-4 border rounded bg-muted text-foreground my-4 mx-auto w-full max-w-[500px] h-[300px] text-sm gap-1 relative overflow-hidden group";

    fallbackDiv.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mb-2 size-6 opacity-90 transition-opacity duration-200 group-hover:opacity-100">
      ...
      <span class="text-sm font-bold tracking-wide">이미지를 불러올 수 없습니다.</span>
      <span class="text-xs font-medium mt-1">주소가 잘못되었거나 원본 파일이 삭제되었을 수 있습니다.</span>
    `;
